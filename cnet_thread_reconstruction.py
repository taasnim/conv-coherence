...
def forward_propagation(X_positive, X_negative, E, mode, print_=True):
    """
    ...
    for d in gpus:
        with tf.device(d):
            ...
            for i, w_size in enumerate(filter_sizes):
                filter_shape = [w_size, opts.filter_width, opts.emb_size, opts.nb_filter]
            ...
            for i, w_size in enumerate(filter_sizes):
                ...
                conv_layer_1_positive = tf.nn.conv2d(embedding_positive, W_conv_layer[i], strides=[1, 1, 1, 1],
                                                    padding="VALID")  # embedding and W_conv_layer[i] both are 4D matrix
                ...
                conv_layer_1_negative = tf.nn.conv2d(embedding_negative, W_conv_layer[i], strides=[1, 1, 1, 1],
                                                    padding="VALID")  # embedding and W_conv_layer_1 both are 4D matrix
                ...
                m_layer_1_positive_inside = tf.nn.max_pool(h_conv_layer_1_positive, ksize=[1, w_size, 2, 1],
                                            strides=[1, w_size, 2, 1], padding="VALID")
                # 1D Pooling for negative document
                m_layer_1_negative_inside = tf.nn.max_pool(h_conv_layer_1_negative, ksize=[1, w_size, 2, 1],
                                            strides=[1, w_size, 2, 1], padding="VALID")
...
if __name__ == '__main__':
    ...
    parser.add_option("-B", "--max_branch", dest="max_branch", type="int",
                      help="Maximum size of the branch of the tree. [default: %default]")
    parser.add_option("-Z", "--filter_width", dest="filter_width", type="int",
                      help="convolution filter width [default: %default]")
    parser.add_option("-W", "--gpu_id", dest="gpu_id",
                      help="GPU IDs. [default: %default]")

    parser.set_defaults(
        data_dir="./data/"
        , log_file="log"
        , model_dir="./checkpoints-02/thread_reconstruction_iter_"
    ...
    entity_type = ['S', 'O', 'X', '-', '0']
    vocabs = new_data_helper.init_vocab(filelist="data/reconstruction-task/list.acl.train_dev", occur=95)
    ...
    X_train_1, X_train_0, vocab_info_train= cnet_helper.load_reconstruction_task_02("data/reconstruction-task/list.acl.train",                                                                
                                                                w_size=w_size,
                                                                vocabs=vocabs,
                                                                maxlen=opts.maxlen, 
                                                                maxbranch=opts.max_branch)
                                                                
    X_dev_1, X_dev_0, vocab_info_dev   = cnet_helper.load_reconstruction_task_02("data/reconstruction-task/list.acl.dev", 
                                                                w_size=w_size,
                                                                vocabs=vocabs,
                                                                maxlen=opts.maxlen, 
                                                                maxbranch=opts.max_branch)

    X_test_1, X_test_0, vocab_info_test    = cnet_helper.load_reconstruction_task_02("data/reconstruction-task/list.acl.test", 
                                                                w_size=w_size,
                                                                vocabs=vocabs,
                                                                maxlen=opts.maxlen, 
                                                                maxbranch=opts.max_branch)
    ...
    ## Create Placeholders
    X_positive = tf.placeholder(tf.int32, shape=[None, opts.maxlen, opts.max_branch])  # Placeholder for positive document
    X_negative = tf.placeholder(tf.int32, shape=[None, opts.maxlen, opts.max_branch])  # Placeholder for negative document
    mode = tf.placeholder(tf.bool, name='mode')  # Placeholder needed for batch normalization

    # Forward propagation
    score_positive, score_negative, parameters = forward_propagation(X_positive, X_negative, E, mode)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(opts.epochs):
            ...
            minibatch_cost = 0.
            num_minibatches = int(num_train/opts.minibatch_size)  # number of minibatches of size minibatch_size in the train set
            ...
            for (i, minibatch_train) in enumerate(minibatches_train):
                ...
                if iter_count%opts.eval_minibatches == 0:
                    #evaluatation here, if acc is better, then save the model
                    #save_model_ each step
                    wins_count = 0
                    ties_count = 0
                    losses_count = 0
                    ...
                    for (j, minibatch_dev) in enumerate(minibatches_dev):
                        ...
                        dev_loss, num_wins, num_ties, num_losses = sess.run([cost, number_wins, number_ties, number_losses],
                                                                  feed_dict={X_positive: minibatch_X_positive,
                                                                             X_negative: minibatch_X_negative,
                                                                             mode: False})
                        wins_count += num_wins
                    ...
                    model_name = opts.model_dir + str(iter_count)
                    saver.save(sess, model_name)
                    print(" -iteration", iter_count, "train-loss:",train_loss, "dev-loss:", dev_loss, "dev-acc:", accuracy)
