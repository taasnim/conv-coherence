...
def forward_propagation(X_positive, X_negative, E, mode, print_=False):
    ...
    with tf.device('/cpu:0'):
        ...
        W_embedding = tf.get_variable("W_embedding", initializer=tf.convert_to_tensor(E, tf.float32))  # embedding matrix
    ...
    for d in gpus:
        with tf.device(d):
            ...
            for i, w_size in enumerate(filter_sizes):
                ...
                m_layer_1_positive_inside = tf.nn.max_pool(h_conv_layer_1_positive, ksize=[1, w_size, 1, 1],
                                            strides=[1, w_size, 1, 1], padding="VALID")
                # 1D Pooling for negative document
                m_layer_1_negative_inside = tf.nn.max_pool(h_conv_layer_1_negative, ksize=[1, w_size, 1, 1],
                                            strides=[1, w_size, 1, 1], padding="VALID")
...
if __name__ == '__main__':
    ...
    parser.set_defaults(
        data_dir="./data/"
        , log_file="log"
        , model_dir="./saved_models/18_02/treelevel/treelevel_pretrained/20_02_treelevel_pretrained_epoch_"
    ...
    print("minibatch_size: ", opts.minibatch_size, "  dropout_ratio: ", opts.dropout_ratio,
          ...
          "  max_branch: ", opts.max_branch, "  filter_width: ", opts.filter_width, "  gpu_id: ", opts.gpu_id)
    ...
    X_train_1, X_train_0, vocab_info_train = cnet_helper.load_data_by_branch_word_entity_extended(filelist="./data/cnet_2500.train",
                                                                                  maxlen=opts.maxlen, w_size=w_size,
                                                                                  vocabs=vocabs,
                                                                                  maxbranch=opts.max_branch)
    print("Loaded Train Dataset")
    X_dev_1, X_dev_0, vocab_info_dev = cnet_helper.load_data_by_branch_word_entity_extended(filelist="./data/cnet_2500.dev",
                                                                            maxlen=opts.maxlen, w_size=w_size,
                                                                            vocabs=vocabs, maxbranch=opts.max_branch)
    print("Loaded dev Dataset")
    X_test_1, X_test_0, vocab_info_test = cnet_helper.load_data_by_branch_word_entity_extended(filelist="./data/cnet_2500.test",
                                                                               maxlen=opts.maxlen, w_size=w_size,
                                                                               vocabs=vocabs,
                                                                               maxbranch=opts.max_branch)
