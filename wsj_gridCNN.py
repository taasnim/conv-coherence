...
def forward_propagation(X_positive, X_negative, E, mode, print_=False):
    ...
    W_conv_layer_1 = tf.get_variable("W_conv_layer_1", shape=filter_shape,
                                     initializer=tf.contrib.layers.xavier_initializer(seed=2018), regularizer=regularizer)  # filter for covolution layer 1
    ...
    h_conv_layer_1_positive = tf.nn.relu(conv_layer_1_with_bn_positive, name="relu_conv_layer_1_positive")  # Apply nonlinearity
    ...
    h_conv_layer_1_negative = tf.nn.relu(conv_layer_1_with_bn_negative, name="relu_conv_layer_1_negative")  # Apply nonlinearity
    ...
    v_fc_layer = tf.get_variable("v_fc_layer", shape=[dim_coherence, 1],
                                 initializer=tf.contrib.layers.xavier_initializer(seed=2018))  # Weight matrix for final layer
...
if __name__ == '__main__':
    ...
    parser.add_option("-M", "--eval_minibatches", dest="eval_minibatches", type="int", help="How often we want to evaluate in an epoch. [default: %default]")
    parser.set_defaults(
        data_dir        = "./data/"
        ,log_file       = "log"
        ,model_dir      = "./saved_models/08_02/gridCNN/"
